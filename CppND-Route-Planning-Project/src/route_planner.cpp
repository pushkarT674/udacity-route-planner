...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for(RouteModel::Node *n : (*current_node).neighbors) {
        ...
        (*n).g_value = (*n).distance(*start_node);
        (*n).visited = true;
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // TODO: Implement your solution here.
  	...
  	std::cout << "Current Node -> g_value" << (*current_node).g_value;
    while (current_node != nullptr) {
      	path_found.insert(path_found.begin(), (*current_node));
      	for(int i=0; i < path_found.size(); i++)
   			std::cout << path_found.at(i).g_value << " ";
      	...
      	if (current_node == end_node) {
        	...
        	AddNeighbors(current_node);
          	...
          	for (int i=0; i < neighbors.size(); i++) {
    			std::cout << "Neighbor " << i << "- g_value" << neighbors[i]->g_value;
      			std::cout << "Neighbor " << i << "- h_value" << neighbors[i]->h_value;
